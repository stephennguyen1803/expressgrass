...
jQuery(document).ready(function(){
	jQuery('#billing\\:postcode, #shipping\\:postcode').blur( function(){
		validatePostcode(jQuery(this));
	})
	jQuery('#billing\\:country_id, #shipping\\:country_id').change( function(){
		if( jQuery(this).attr("id") == "billing:country_id" ){
			target = "#billing\\:postcode";
		}else{
			target = "#shipping\\:postcode";
		}
		validatePostcode(jQuery(target));
	})
	jQuery('#onestepcheckout-place-order').click( function(){
		validatePostcode(jQuery("#billing\\:postcode"));
		validatePostcode(jQuery("#shipping\\:postcode"));
	})	
})

function validatePostcode(obj){
	var field= obj;
	if(obj.attr("id") == "billing:postcode"){
		var country_field = "#billing\\:country_id";
	}else{
		var country_field = "#shipping\\:country_id";
	}
	if( jQuery(country_field).val() != "" && obj.val() != "" ){
		if(jQuery(country_field).val() == "GB"){
			var newPostCode = checkPostCode (jQuery.trim(obj.val()));
			if (newPostCode) {
				obj.val(newPostCode);
				jQuery('#advice-required-entry-postcode').remove();
			}else{ 	
				if(jQuery('#advice-required-entry-postcode').length == 0 ){
					jQuery('<div class="validation-advice" id="advice-required-entry-postcode">This Postcode is invalid.</div>').insertAfter( obj );
				}
			}
		}
	}
