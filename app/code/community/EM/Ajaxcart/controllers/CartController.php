        ...
        try {
            ...
            if (!$this->_getSession()->getNoCartRedirect(true)) {
                if (!$cart->getQuote()->getHasError()){
					...
					<div id="viewcart_button_msg">'.Mage::helper('checkout')->__('View cart &amp; checkout').'</div>';
					Mage::register('ajaxcart_message',$tmp_product);
                }
            }
        } catch (Mage_Core_Exception $e) {
           if ($this->_getSession()->getUseNotice(true)) {
				Mage::register('ajaxcart_message','<div id="message_ajax">'.Mage::helper('core')->escapeHtml($e->getMessage()).'</div>');
            } else {
                Mage::register('ajaxcart_message','<div id="message_ajax">'.Mage::helper('core')->escapeHtml($e->getMessage()).'</div>');
            }
        } catch (Exception $e) {
