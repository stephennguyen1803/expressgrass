...
class EM_Blog_Block_Post_View extends Mage_Core_Block_Template
{
	...
	protected function initAllowComment(){
		...
		if($allow_comment == 0) /* Only logged user can comment */
		{
			if($this->helper('customer')->isLoggedIn())
				$this->setAllowComment(1);/* allow comment */
			else
				$this->setAllowComment(0);/* not allow comment */
		}
		elseif($allow_comment == 1) /* Every one can comment */
			$this->setAllowComment(1);/* allow comment */
		else /* Anybody can't comment */
			$this->setAllowComment(0);/* not allow comment */
		return $this;	
	}
}
