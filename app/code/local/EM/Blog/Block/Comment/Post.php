...
class EM_Blog_Block_Comment_Post extends Mage_Core_Block_Template
{
    ...
    public function  _prepareLayout() {
		...
		$limit = Mage::getStoreConfig('blog/info/limit_comment_pagination');
		$toolbar->addPagerLimit('detail',$limit,$limit);
    ...
    {
        $textIndent = $level*20;
        $date = new DateTime($comment['time_comment']);
                     ...
                     <div class='comment-item-header'><span>".$comment['username']."</span>
                             <div class='comment-status'>
                                    ...
                                    <div class='comment-button'>
                                            <button name='reply'  class='button' value='Reply' com_id='".$comment['id']."' onclick='addCommentBox(".$comment['id'].",".$comment['id'].");'><span><span>".$this->__('Reply')."</span></span></button>
                                    </div>
                             </div>
                     </div>
