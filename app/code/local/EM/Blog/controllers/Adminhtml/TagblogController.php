<?php
class EM_Blog_Adminhtml_TagblogController extends Mage_Adminhtml_Controller_Action
{
	/**
    ...
    protected function _isAllowed()
    {
        switch ($this->getRequest()->getActionName()) {
            case 'new':
            case 'save':
                return Mage::getSingleton('admin/session')->isAllowed('cms/tag_manager/save');
                break;
            case 'delete':
                return Mage::getSingleton('admin/session')->isAllowed('cms/tag_manager/delete');
                break;
            default:
                return Mage::getSingleton('admin/session')->isAllowed('cms/tag_manager');
                break;
        }
    }
	...
	protected function _initAction() {
		$this->loadLayout()
			->_setActiveMenu('cms/tag_manager')
	...
	public function editAction() {
		...
		if ($model->getId() || $id == 0) {
			...
			$this->loadLayout();
			$this->_setActiveMenu('cms/tag_manager');

			$this->_addBreadcrumb(Mage::helper('adminhtml')->__('Tags Manager'), Mage::helper('adminhtml')->__('Tags Manager'));
			
			$this->getLayout()->getBlock('head')->setCanLoadExtJs(true);

			$this->_addContent($this->getLayout()->createBlock('blog/adminhtml_tagblog_edit'))
				->_addLeft($this->getLayout()->createBlock('blog/adminhtml_tagblog_edit_tabs'));
    ...
    {
        $fileName   = 'tag.csv';
        $content    = $this->getLayout()->createBlock('blog/adminhtml_tagblog_grid')
            ->getCsv();
