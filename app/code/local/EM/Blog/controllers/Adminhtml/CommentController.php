...
class EM_Blog_Adminhtml_CommentController extends Mage_Adminhtml_Controller_Action
{
    ...
    protected function _isAllowed()
    {
        switch ($this->getRequest()->getActionName()) {
            case 'new':
            case 'save':
                return Mage::getSingleton('admin/session')->isAllowed('cms/comment_manager/save');
                break;
            case 'delete':
                return Mage::getSingleton('admin/session')->isAllowed('cms/comment_manager/delete');
                break;
            default:
                return Mage::getSingleton('admin/session')->isAllowed('cms/comment_manager');
                break;
        }
    }
            ...
            try {
                foreach ($postIds as $postId) {
                    $blog = Mage::getSingleton('blog/comment')
                        ->load($postId)
                        ->setStatusComment($this->getRequest()->getParam('status'))
