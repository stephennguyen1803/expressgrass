...
class EM_Blog_Adminhtml_CategoryController extends Mage_Adminhtml_Controller_Action
{
    ...
    protected function _isAllowed()
    {
        switch ($this->getRequest()->getActionName()) {
            case 'new':
            case 'save':
                return Mage::getSingleton('admin/session')->isAllowed('cms/category_manager/save');
                break;
            case 'delete':
                return Mage::getSingleton('admin/session')->isAllowed('cms/category_manager/delete');
                break;
			case 'move':
                return Mage::getSingleton('admin/session')->isAllowed('cms/category_manager/move');
                break;	
            default:
                return Mage::getSingleton('admin/session')->isAllowed('cms/category_manager');
                break;
        }
    }
...
/*            Mage::dispatchEvent('catalog_category_prepare_save', array(
    ...
    public function moveAction()
    {
		...
		if($category->getParentId() != $parentNodeId){
			...
			if($urlInstance->validate($newUrlPath,'category_id',$category->getId()))
