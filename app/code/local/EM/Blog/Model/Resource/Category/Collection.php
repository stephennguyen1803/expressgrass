<?php
class EM_Blog_Model_Resource_Category_Collection extends EM_Blog_Model_Resource_Collection_Abstract
{
	 /**
     * Event prefix
     *
     ...
     * @var boolean
     */
    protected $_loadWithProductCount     = true;
    ...
    protected function _construct()
    {
        ...
        /* $this->_productWebsiteTable = $this->getTable('blog/product_website'); */
        $this->_productTable        = $this->getTable('blog/category_post');
    }
    ...
    public function joinUrlRewrite()
    {
        ...
        $this->joinTable(
            ...
            array('request_path'),
                "{{table}}.post_id IS NULL"
                . " AND {{table}}.tag_id IS NULL",
            'left'
        );
        return $this;
    }
        ...
        if ($countRegular) {
            ...
            if (!empty($regularIds)) {
                ...
                $select->from(
                        ...
                        array('category_id', new Zend_Db_Expr('COUNT(main_table.post_id)'))
                    )
                ...
                /*if ($websiteId) {
                    $select->join(
                        ...
                        'main_table.post_id = w.post_id', array()
                    )
        ...
        if ($countAnchor) {
            ...
            foreach ($anchor as $item) {
                if ($allChildren = $item->getAllChildren()) {
                    ...
                    $select->from(
                            array('main_table' => $this->_productTable),
                            new Zend_Db_Expr('COUNT(DISTINCT main_table.post_id)')
                        )
                        ->joinInner(
                            array('e' => $this->getTable('blog/category')),
