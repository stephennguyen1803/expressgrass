...
class EM_Blog_Model_Post extends EM_Blog_Model_Abstract
{
	...
	*/
    ...
    public function getPostUrl($includeCat = true){
        if(($cat = Mage::registry('current_cat')) && $includeCat){
            if(in_array(Mage::registry('current_cat')->getId(),$this->getCategoryIds()))
				$prefixUrl = '/'.$cat->getPathUrl().'/';
			else
				$prefixUrl = '';
        } else {
            $prefixUrl = '';
        }
		return Mage::helper('blog')->getBlogUrl($prefixUrl.$this->getPostIdentifier().'.html');
    }
}
?>